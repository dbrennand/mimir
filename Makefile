...
push-multiarch-mimir:
	...
	$(MAKE) GOOS=linux GOARCH=arm BINARY_SUFFIX=_linux_arm cmd/mimir/mimir
	$(SUDO) docker buildx build -o type=registry --platform linux/amd64,linux/arm64,linux/arm/v7 --build-arg=revision=$(GIT_REVISION) --build-arg=goproxyValue=$(GOPROXY_VALUE) --build-arg=USE_BINARY_SUFFIX=true -t $(IMAGE_PREFIX)mimir:$(IMAGE_TAG) cmd/mimir
